...
class ValorantAPI:
    ...
    def store_night_offers(self) -> dict:
        data = self.fetch("/store/v2/storefront/{user_id}".format(user_id=self.user_id))
        return data["BonusStore"]
    
    def store_night_price(self) -> dict:
        data = self.fetch('/store/v1/offers/')
        return data['DiscountCosts']
    ...
    def my_night_offter(self) -> None:
        skinid = self.store_night_offers()
        get_price = self.store_night_price()
        skin = []
        icon = []
        price = []
        for i in skinid:
